...
Lees het bestand `config/bronnen.json` in de repo. Gebruik alleen de bronnen waar `scan_task` gelijk is aan `"A"`.
...
Maak of overschrijf het bestand `werk/scan-a.json` in de repo op branch `main` met dit formaat:
...
Commit message: `scan-a: nieuws YYYY-MM-DD`
